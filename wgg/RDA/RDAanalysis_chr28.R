...
# --- Filter data to focus on a single chromosome (e.g., chromosome 28) ---
target_chromosome <- 28
bim_data_chr28 <- bim_data[bim_data$CHR == target_chromosome, ]
cat("Total SNPs found on Chromosome", target_chromosome, ":", nrow(bim_data_chr28), "\n")
if (nrow(bim_data_chr28) == 0) {
...
snp_ids <- paste0(bim_data_chr28$CHR, "_", bim_data_chr28$POS)

# Filter the genotype data to keep only SNPs on chromosome 28
...
# --- Diagnostic Step ---
cat("Total SNPs loaded for Chromosome 28:", ncol(genetic_data_final), "\n")
...
write.csv(snp_scores_df, "rda_snp_scores_chr28.csv", row.names = FALSE)

# Save the list of selected SNPs for mapping
selected_snps_df <- data.frame(SNP_ID = colnames(genetic_data_final))
write.csv(selected_snps_df, "selected_snps_for_plotting_chr28.csv", row.names = FALSE)

cat("RDA results exported to 'rda_snp_scores_chr28.csv' and 'selected_snps_for_plotting_chr28.csv'\n")
