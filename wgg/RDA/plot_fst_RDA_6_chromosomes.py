...
for chrom in TARGET_CHROMOSOMES:
    ...
    # --- REVISED CALCULATION: STANDARD RDA1 MAGNITUDE ---
    # In standard RDA, the first axis captures the primary speciation/ecological split.
    ...
    summary_results.append({
        ...
        'P-Value_RDA': f"{pval_rda:.4f}" if isinstance(pval_rda, float) else pval_rda
    })
    ...
    ax2.scatter(x=sig_rda['POS'], y=sig_rda['distance'], s=30, c='red', zorder=3, label=f'Top {100-VISUAL_THRESHOLD_QUANTILE*100:.0f}% RDA1 Outliers')
    ...
    ax2.set_title(f"Chromosome {chrom}: Standard RDA Manhattan Plot", fontsize=14)
    ax2.legend(loc='upper right')
    
    plt.tight_layout()
    plt.savefig(f"multipanel_genomic_plot_chr{chrom}_standard_RDA.png", dpi=300)
    plt.savefig(f"multipanel_genomic_plot_chr{chrom}_standard_RDA.pdf", format='pdf', dpi=300)
    plt.close(fig) # Frees up memory after each loop iteration

# --- STEP 3: PRINT SUMMARY TABLE ---
print("\n\n" + "="*60)
print("FINAL GENOME-WIDE STATISTICAL SUMMARY (STANDARD RDA)")
print("="*60)
print(f"{'Chrom':<10} | {'SVs Found':<12} | {'Fst P-Value (Spec)':<20} | {'RDA1 P-Value (Spec)':<20}")
print("-" * 60)
for res in summary_results:
    print(f"{res['Chromosome']:<10} | {res['SVs_Found']:<12} | {res['P-Value_Fst']:<20} | {res['P-Value_RDA']:<20}")
print("="*60)
