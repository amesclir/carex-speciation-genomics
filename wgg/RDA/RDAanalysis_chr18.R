...
# --- Filter data to focus on a single chromosome (e.g., chromosome 18) ---
target_chromosome <- 18
bim_data_chr18 <- bim_data[bim_data$CHR == target_chromosome, ]
cat("Total SNPs found on Chromosome", target_chromosome, ":", nrow(bim_data_chr18), "\n")
if (nrow(bim_data_chr18) == 0) {
...
snp_ids <- paste0(bim_data_chr18$CHR, "_", bim_data_chr18$POS)

# Filter the genotype data to keep only SNPs on chromosome 18
...
# --- Diagnostic Step ---
cat("Total SNPs loaded for Chromosome 18:", ncol(genetic_data_final), "\n")
...
write.csv(snp_scores_df, "rda_snp_scores_chr18.csv", row.names = FALSE)

# Save the list of selected SNPs for mapping
selected_snps_df <- data.frame(SNP_ID = colnames(genetic_data_final))
write.csv(selected_snps_df, "selected_snps_for_plotting_chr18.csv", row.names = FALSE)

cat("RDA results exported to 'rda_snp_scores_chr18.csv' and 'selected_snps_for_plotting_chr18.csv'\n")
