...
for (target_chromosome in target_chromosomes) {
  ...
  # Run the Standard RDA
  cat("Running Standard RDA for Chromosome", target_chromosome, "...\n")
  rda_result <- rda(genetic_data_final ~ ., data = as.data.frame(bioclim_scaled))
  
  # --- NEW: PRINT STATISTICS ---
  adj_r2 <- RsquareAdj(rda_result)
  cat("\nAdjusted R-squared for Chromosome", target_chromosome, ":\n")
  print(adj_r2)
  ...
  # Export RDA results for Python plotting
  cat("\nExporting RDA results for Python plotting...\n")
