...
q_file_path = "admixture_results/admixture_chr14_inversion_final_ld_pruned.2.Q"
...
output_plot_base = "admixture_chr14_plot_K2_individuals"
...
try:
    ...
    # Plotting
    # Increased figure size for visibility of many individuals
    ...
    for i in range(admixture_proportions.shape[1]):
        ax.barh(merged_data.index, admixture_proportions[:, i], left=left, color=colors[i], height=1.0)
    ...
    for boundary, pop_label in zip(np.cumsum(pop_boundaries), unique_pops):
        ax.text(1.01, boundary - (pop_boundaries[np.where(unique_pops == pop_label)[0][0]] / 2),
                pop_label, verticalalignment='center', rotation=270, fontsize=12,
    ...
    print(f"ADMIXTURE plot with individual labels for K=2 saved to {output_plot_base}.png")
    
    # Save as PDF (vector format)
    plt.savefig(f"{output_plot_base}.pdf")
    print(f"ADMIXTURE plot with individual labels for K=2 saved to {output_plot_base}.pdf")
