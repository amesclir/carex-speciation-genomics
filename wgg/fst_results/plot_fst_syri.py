...
rearrangement_types = ['INVTR', 'DEL', 'INTR', 'INTRAL', 'INS', 'DUP']
...
SIZE_THRESHOLD = 50000  # Adjust this value as needed (e.g., 50000 bp)
...
ax.scatter(x=df_fst['BPcum'], y=df_fst['FST'], s=2, c=df_fst['color'], zorder=1)
...
ax.scatter(x=df_outliers['BPcum'], y=df_outliers['FST'], s=10, c='red', zorder=2)
...
ax.set_ylim(0.9, df_fst['FST'].max() + 0.05)

# Add labels and a title to the plot
ax.set_xlabel("Chromosome", fontsize=12)
ax.set_ylabel("Fst", fontsize=12)
ax.set_title(f"Fst Values for POP1 vs POP2 (rearrangements > {SIZE_THRESHOLD}bp)", fontsize=16)

# Save the plot to a new file in the current directory
plt.savefig(f"fst_manhattan_plot_high_values_significant_rearrangements.png")
