...
def main():
    ...
    generates density plots, and runs regression models.
    """
    ...
    # Select top K for individual plotting
    ...
    df_windows_by_chr = {chr_int: df_windows[df_windows['CHR_int'] == chr_int].copy() 
                         for chr_int in df_windows['CHR_int'].unique()}
    ...
    # --- SECTION 3B: Global SV vs Non-SV Comparison & Plotting ---
    print("\n" + "="*80)
    print("--- 3B. Global Test & Plot: All Large SVs vs. Rest of Genome ---")
    print("="*80)
    
    all_sv_windows = []
    
    # Iterate over ALL rearrangements > 10kb (not just Top 30)
    ...
    # Background is everything NOT in df_all_sv
    ...
    print("\nGenerating Global SV vs Genome Plot...")
    fig, axes = plt.subplots(5, 1, figsize=(10, 20))
    axes = axes.flatten()
    
    global_q95_bg_ratio = df_all_bg['PI_RATIO'].dropna().quantile(0.95)
    global_standard_x_limit = max(global_q95_bg_ratio, 2.0)
    
    for i, (stat_col, config) in enumerate(METRICS_TO_TEST.items()):
        ax = axes[i]
        
        # Plot Global Background
        sns.kdeplot(df_all_bg[stat_col].dropna(), ax=ax, color='#333333', fill=True, alpha=0.6, label='Genome Background')
        # Plot All SVs Aggregate
        sns.kdeplot(df_all_sv[stat_col].dropna(), ax=ax, color=config['color'], fill=True, alpha=0.6, label='All Rearrangements (>10kb)')
        
        ax.axvline(df_all_bg[stat_col].mean(), color='#333333', linestyle='--', linewidth=1, label='Genome Mean')
        ax.axvline(df_all_sv[stat_col].mean(), color=config['color'], linestyle='-', linewidth=1, label='SV Mean')
        
        ax.set_title(f"Global: {config['label']}") 
        ax.set_xlabel(config['label'])
        
        if stat_col == 'PI_RATIO':
            ax.axvline(1.0, color='black', linestyle=':', linewidth=1)
            ax.set_xlim(0, global_standard_x_limit)
        
        ax.legend(loc='upper right', fontsize='small')

    plt.suptitle(f'Global Profile: All Rearrangements > 10kb vs Genome', fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    
    global_plot_path = os.path.join(PLOTS_OUTPUT_DIR, "Global_All_SVs_vs_Genome.png")
    plt.savefig(global_plot_path, dpi=300)
    plt.close(fig)
    print(f"Saved global plot to {global_plot_path}")

    # --- SECTION 4: Density Plotting (Individual) ---
    print("\n" + "="*80)
    print(f"--- Generating 5-Panel Vertical Density Plots for TOP {K_TOP_SVS} SVs in '{PLOTS_OUTPUT_DIR}/' ---")
    print("="*80)
    ...
    # --- SECTION 5: Regression Analysis (FST ~ DXY + SIZE) ---
    print("\n" + "="*80)
    print("--- 5. Linear Regression Models (All SVs > 10kb) ---")
    print("="*80)

    regression_data = []

    # Iterate over ALL rearrangements > 10kb
    for _, sv_row in df_rearrangements.iterrows():
        chr_int = sv_row['CHR_int']
        sv_start_safe = min(sv_row['START'], sv_row['END'])
        sv_end_safe = max(sv_row['START'], sv_row['END'])
        ...
        # We need at least 1 window to calculate a mean
        if len(sv_loci) < 1:
            continue

        # Collect data for this SV
        sv_stats = {
            'SIZE': float(sv_row['SIZE']),
            'FST': sv_loci['FST'].mean(),
            'DXY': sv_loci['DXY'].mean()
        }
        regression_data.append(sv_stats)

    df_reg = pd.DataFrame(regression_data)
    
    # Drop rows with NaN (if any SV had windows but NaN FST/DXY)
    df_reg.dropna(subset=['FST', 'DXY', 'SIZE'], inplace=True)

    print(f"Running regression on {len(df_reg)} SVs...")

    if len(df_reg) > 2:
        # Model 1: FST ~ DXY
        print("\n>>> Model 1: FST ~ DXY")
        model1 = smf.ols("FST ~ DXY", data=df_reg).fit()
        print(model1.summary())

        # Model 2: FST ~ SIZE
        print("\n>>> Model 2: FST ~ SIZE")
        model2 = smf.ols("FST ~ SIZE", data=df_reg).fit()
        print(model2.summary())

        # Model 3: FST ~ DXY + SIZE
        print("\n>>> Model 3: FST ~ DXY + SIZE")
        model3 = smf.ols("FST ~ DXY + SIZE", data=df_reg).fit()
        print(model3.summary())
    else:
        print("Not enough data points to run regression models.")
