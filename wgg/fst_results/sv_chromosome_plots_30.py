...
def load_data_and_svs():
    ...
    # --- FIX APPLIED HERE: Removed .head(10) to include all relevant SVs ---
...
def plot_chromosome_data(df_chr, sv_info, chr_int, output_dir):
    ...
    METRICS = [
        ...
        {'col': 'PI_POP1', 'label': '$\\pi$ Pop1 (Diversity)', 'color': 'cyan', 'ylim_max_q': 0.99},
...
def main():
    ...
    # Target chromosomes based on user's important SVs (SV1, SV2, SV3, SV4, SV8, SV16, SV23)
    TARGET_CHRS = {
        14: 'SV1',
        18: 'SV2',
        20: 'SV3',
    ...
    for chr_int, sv_id_target in TARGET_CHRS.items():
        ...
        # --- FIX APPLIED HERE: Check if the SV was found ---
        if sv_info_match.empty:
            print(f"Error: Target SV ID '{sv_id_target}' (Chr{chr_int}) not found in the loaded SV list (check size threshold: {df_top_svs.SIZE.min()} bp). Skipping.")
            continue
