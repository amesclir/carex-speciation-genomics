...
def plot_chromosome_data(df_chr, sv_info, chr_int, output_dir):
    ...
    # ORDER: FST -> DXY -> PI_BORB -> PI_BORY -> RATIO
    METRICS = [
        {'col': 'FST', 'label': '$F_{ST}$ (Divergence)', 'color': 'darkred', 'ylim_max': 1.0},
        {'col': 'DXY', 'label': '$D_{xy}$ (Abs Divergence)', 'color': 'red'}, # Added Dxy in Red
    ...
    for i, metric in enumerate(METRICS):
        ...
        if col == 'FST':
            ax.set_ylim(0, metric['ylim_max'])
             ...
             ax.set_ylim(0, dxy_max * 1.1) # Add a little headroom
