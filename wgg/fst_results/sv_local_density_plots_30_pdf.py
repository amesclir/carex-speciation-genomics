...
def main():
    ...
    print("\n" + "="*80)
    print(f"--- Generating 4-Panel Density Plots in '{PLOTS_OUTPUT_DIR}/' (PNG and PDF) ---")
    print("="*80)
    ...
    for _, sv_row in df_top_svs.iterrows():
        ...
        # Save as PNG
        png_filename = os.path.join(PLOTS_OUTPUT_DIR, f"{sv_row['SV_ID']}_{sv_row['TYPE']}_Chr{chr_int}_local_analysis.png")
        plt.savefig(png_filename, dpi=300)
        print(f"Generated {png_filename}")

        # Save as PDF (New addition)
        pdf_filename = os.path.join(PLOTS_OUTPUT_DIR, f"{sv_row['SV_ID']}_{sv_row['TYPE']}_Chr{chr_int}_local_analysis.pdf")
