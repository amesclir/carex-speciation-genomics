...
Pipeline for processing short-read sequencing data (184 individuals), variant calling, and population-level analyses.
    ...
    fst_results/ & ld_decay/: Scripts for calculating windowed and per-SNP FST​, nucleotide diversity (π), dXY​, and matched Linkage Disequilibrium (LD) decay inside vs. outside structural variants.
...
Custom pipeline to calculate evolutionary rates (ω=dN/dS) and neutral substitution rates (P4D) inside vs. outside structural variants to test for Hill-Robertson interference / relaxed purifying selection.
