...
# Now configured with the absolute path based on your 'genomecbory.fa' location:
FASTA_FAI_PATH = '/home/aescudero/genome_assembly/B6_clean_assemblies/C_boryana/C_boryana.fasta.fai'

# Your existing, correct paths for annotations:
GENE_GFF_PATH = '/home/aescudero/genome_assembly/C2_braker/braker_cbory/braker.gff3'
REPEAT_GFF_PATH = '/home/aescudero/genome_assembly/C3_earlgrey/C_boryana/C_boryana_EarlGrey/C_boryana_summaryFiles/C_boryana.filteredRepeats.gff'
OUTPUT_FILENAME = 'carex_boryana_genomic_data.json'

# Fixed window size for genomic bins (Updated to 100,000 bp / 0.1 Mb)
FIXED_BIN_SIZE_BP = 100000

# --- 2. Track Definitions (UPDATED for Copia and Gypsy) ---
...
def load_contig_lengths(fai_path):
    ...
    try:
        ...
        print("Please check and ensure 'genomecbory.fa.fai' exists in that directory.")
        sys.exit(1)
...
def calculate_gene_density(gff_path, genomic_bins):
    """Calculates gene count per bin (normalized)."""
    ...
    try:
        with open(gff_path, 'r') as f:
            for line in f:
                ...
                try:
                    parts = line.split('\t')
                    # Check for minimal GFF fields and feature type
    ...
    try:
        with open(gff_path, 'r') as f:
            for line in f:
                ...
                # --- NEW LOGIC TO MAP REPEAT CLASS TO REQUESTED TRACKS ---
                track_name = None
